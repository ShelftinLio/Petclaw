...
4. [安装 Petclaw 桌面龙虾](#4-安装-petclaw-桌面龙虾)
5. [配置语音系统](#5-配置语音系统)
6. [接入飞书](#6-接入飞书)
7. [接入 Telegram](#7-接入-telegram可选)
8. [配置 Petclaw Switch 模型热切换](#8-配置-petclaw-switch-模型热切换可选)
9. [个性化配置](#9-个性化配置)
...
Petclaw 基于 Electron，需要 Node.js 环境。
...
Petclaw 是 OpenClaw 的桌面可视化伴侣，需要先安装 OpenClaw。
...
1. 访问 [GitHub Releases](https://github.com/kk43994/petclaw/releases/latest)
2. 下载对应系统的安装包：
   - Windows: `Petclaw-Desktop-Pet-x.x.x-Setup.exe`
   - macOS Intel: `Petclaw-Desktop-Pet-x.x.x-x64.dmg`
   - macOS Apple Silicon: `Petclaw-Desktop-Pet-x.x.x-arm64.dmg`
3. 安装后启动
...
> 💡 **小贴士**：Petclaw 内置了智能情感识别（`smart-voice.js`），会根据文本内容自动选择合适的情感，无需手动指定。
...
> 💡 即使配了多个引擎，Petclaw 会按优先级自动选择：MiniMax → CosyVoice → Edge TTS。如果高优先级引擎报错，会自动降级到下一个。
...
## 8. 配置 Petclaw Switch 模型热切换（可选）

Petclaw Switch 可以在不重启的情况下，3秒切换 AI 模型。

### 8.1 工作原理

```
你在界面上点击切换 → Petclaw 写入数据库 → 自动同步监听器检测变化
→ 更新 OpenClaw 配置 → Gateway 自动重启 → 新模型生效
```

### 8.2 自动模式（默认）

Petclaw 启动时会自动开启同步监听器，无需额外配置。
...
```bash
cd petclaw
node petclaw-hotswitch.js              # 同步当前 provider
node petclaw-hotswitch.js --restart    # 同步 + 重启 Gateway
```

### 8.4 配置多个模型

在 `~/.openclaw/openclaw.json` 中配置多个 provider，然后就可以在 Petclaw Switch 中自由切换：
...
3. 或在终端执行：
   ```bash
   xattr -cr /Applications/Petclaw-Desktop-Pet.app
   ```
...
配置多个 provider 后，使用 Petclaw Switch 功能一键切换。详见 [第8节](#8-配置-petclaw-switch-模型热切换可选)。
...
**遇到问题？**
- 📖 [GitHub Issues](https://github.com/kk43994/petclaw/issues)
